...
#define CHUNK_SIZE 8 // iassumed fixed size of chunk to be 8
...
int send_message(int sock, struct sockaddr_in *receiver_addr, const char *message)
{
   ...
   for (size_t seq = 0; seq < total_chunks; seq++) {
    ...
    {
        printf("Error or no ACK received from client.\n");
...
int receive_message(int sock, struct sockaddr_in *client_addr)
{
    ...
    while (1)
    {
        int received = recvfrom(sock, &total_chunks, sizeof(total_chunks), 0, (struct sockaddr *)client_addr, &addr_len);
        if (received > 0)
        {
    ...
    while (chunks_received < total_chunks || !all_chunks_acknowledged(acked_chunks, total_chunks))
    {
        ...
        int received = recvfrom(sock, &packet, sizeof(packet), 0, (struct sockaddr *)client_addr, &addr_len);
    ...
    char ack_msg[] = "ACK";
    sendto(sock, ack_msg, strlen(ack_msg), 0, (struct sockaddr *)client_addr, sizeof(*client_addr));
...
int main()
{
    ...
    while (1)
    {
        ...
        printf("Server: Enter the message to send: ");
        ...
        receive_message(sock, &receiver_addr);
    }
